...
public class OriginalFastaReader{
	...
	public OriginalFastaReader(String filename){
...
//-					System.out.println("name: " + line);
					...
					if (dataLine.length() > 0) {
						data.add(this.codeData(dataLine));
					}
					dataLine = new String();
				} else {
//c					(a sequence)				
...
*/					dataLine += line;
	...
	public ArrayList<Integer> codeData(String inputChars){
		char [] sequence = inputChars.toCharArray();
		ArrayList<Integer> coded = new ArrayList<Integer>();
		for(char c: sequence){
			coded.add(this.getCode(c));
		}
		return coded;
	}
