@data;
open(DATA, $ARGV[1]) or die "can't open $ARGV[1]\n";
while(<DATA>){
	chomp($line=$_);
	push(@data, $line);
	print "data: $line\n";
...
foreach $site(@data){
	...
	if($fields[0] ne ''){
		print "$fields[0]\n";
		push(@newData,"$fields[0]\t$fields[1]");
	}else{
		print "\a";
	}
}
@data = @newData;
print "after pruning ".scalar(@data)." codons of data.\n(e.g. $data[0] ".$data[scalar($data)-1].")\n";
...
$filename = "$infile.".$ARGV[2].".2011.pdb";
open(PRECOLOUR, '>', "$filename") or die "can't open $filename\n";
...
#		@evolData = split(/\t/,$data[$residue-1]); #IMPORTANT!!!!! old indices $residue-1 ---- now able to use $residue
		@evolData = split(/\t/,$data[$residue]); #IMPORTANT!!!!! old indices $residue-1 ---- now able to use $residue
...
#		print "$aggEpis\n";
		...
		$preColourVal = $evolData[1];
		substr($preColour, 61) = $preColourVal; 

		if($fields[2] eq 'N'){
			print "$evolData[1]|\t$preColour\n";
		}
